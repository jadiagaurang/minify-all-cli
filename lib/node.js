...
module.exports = class MinifyAllCLI {
  ...
  process() {
    ...
    arrayAllFiles.forEach(strFilePath => {
        ...
        console.log(strPartialFilePath);
        ...
        console.log(strDestinationFile);
        ...
        if (!me.options.skipJS && 
          ...
          AppUtil.doGzip(strFilePath, strDestinationFile)
          .then(function () {
            me.logger.warn("Skip. Gzipped! Path: " + strDestinationFile);
          })
          .catch((err) => {
            if (err) me.logger.error(err);  //throw err;
            process.exitCode = 1;
